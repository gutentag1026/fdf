...
/*   Updated: 2018/06/13 01:57:05 by yhuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		valid_map(char	*line, int i, int *cols, int *tmp_nbr)
{
	...
	while (line[++i])
		{
			if (line[i] == ',' && line[i + 1] == '0' && line[i + 2] == 'x' && is_hex(&line[i + 3]))
			{	
			//	printf("%d\n",is_hex(&line[i + 3]));	
				...
				return (1);
			if (line[i] == ' ' && (ft_isdigit(line[i - 1]) || (line[i - 1] >= 'a' && line[i - 1] <= 'f') || (line[i - 1] >= 'A' && line[i - 1] <= 'F')))
				(*cols)++;
		}
		if (ft_isdigit(line[i - 1]) || (line[i - 1] >= 'a' && line[i - 1] <= 'f') || (line[i - 1] >= 'A' && line[i - 1] <= 'F'))
			(*cols)++;
...
void	read_from_file(char *line, int fd, t_map_preprocess *pre, int i)
{
	...
	while (get_next_line(fd, &line) > 0)
	{
		...
		{
			ft_putstr("invalid map!");//map characters could only be ' ', '-',and digit,after '-', it could only be digit, before '-' it could only be space.
			exit (0);
		}
		tmp_nbr = pre->cols;
